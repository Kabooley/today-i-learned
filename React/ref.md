...
-   [ref を使っていいタイミング](#ref-を使っていいタイミング)
...
`ref`を DOM 参照に使う場合、`ref`が DOM にアタッチされるのはいつか？

-   初期レンダリング時：DOM はまだ作成されていないから ref は null。
-   毎レンダリング時: ref は更新されない。
-   コミット時（変更を DOM に適用するとき）: ref は DOM にアタッチされる(ref.current へ DOM が渡される)

ということで、レンダリング中に`ref`を読み取ったり書き込んだりしてはならない。上述の通りレンダリング中は変更がまだ適用されていないからである。DOM を参照する使い方に限らず、`ref`はレンダリング中に参照してはならない。

https://react.dev/reference/react/useRef#caveats

https://react.dev/learn/synchronizing-with-effects#step-1-declare-an-effect

必然的に、`ref`を読み書きするタイミングはレンダリング後、もしくはイベントハンドラ内である。
...
NOTE: `useEffect()`はそのうちに使う変数などは依存関係に含めないといけないが、`ref.current`は例外といえるようで、`ref.current`を依存関係に含めなくても使うことができる。
...
`ref.current`に変更があったら`useEffect(, [ref.current])`で変更を検知しようという考えだ。
...
例：任意の button をクリックするとその button の番号を ref に渡し、その変更を取得しようとして失敗しているコード
...
> これは、ref オブジェクトが static アイデンティティを持っているためです。React は、レンダリングごとに同じ useRef 呼び出しから常に同じオブジェクトを取得することを保証します。これは決して変更されないため、それ自体でエフェクトが再実行されることはありません。したがって、含めても含めなくても問題ありません。それを含めても問題ありません:
...
#### `ref.current`の変更時に再レンダリングをトリガーさせる
...
(この例だと、`useState`使っているからかえって ref を使う意味がさっぱりですが。)
...
一方`useEffect`を使う場合と異なり、必ずレンダリング時毎回`Callback ref`が呼び出され制御できない。

公式で示された方法: mount 時と unmount 時にのみ呼び出される`Callback ref`
...
## 親コンポーネントから ref を取得し、なおかつ自コンポーネントも useRef()で ref を使いたいとき
